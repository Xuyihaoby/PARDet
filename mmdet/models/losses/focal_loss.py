...
def sigmoid_focal_loss_uncertain(pred,
                       target,
                       weight=None,
                       gamma=2.0,
                       alpha=0.25,
...
@LOSSES.register_module()
class FocalLossUncertain(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            loss_weight (float, optional): Weight of loss. Defaults to 1.0.
        """
        super(FocalLossUncertain, self).__init__()
        assert use_sigmoid is True, 'Only sigmoid focal loss supported now.'
        self.use_sigmoid = use_sigmoid
        self.gamma = gamma
        self.alpha = alpha
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        if self.use_sigmoid:
            loss_cls = self.loss_weight * sigmoid_focal_loss_uncertain(
                pred,
                target,
                weight,
                gamma=self.gamma,
