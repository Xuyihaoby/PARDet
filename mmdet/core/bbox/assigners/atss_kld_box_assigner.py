...
from ..rtransforms import gt2gaussian, obb2poly


@BBOX_ASSIGNERS.register_module()
class ATSSKldBoxAssigner(BaseAssigner):
    ...
    def __init__(self, topk, angle_version, use_reassign=False):
        self.topk = topk
    ...
    def assign(self,
        ...
        overlaps = self.kld_overlaps(gt_bboxes, bboxes)  # (num_bbox, num_gt)
        ...
        bboxes_points = bboxes[:, :2].contiguous()
    ...
    def kld_overlaps(self, gt_rbboxes, bboxes, eps=1e-6):
        ...
        """
        assert bboxes.shape[-1] == 5 and gt_rbboxes.shape[-1] == 5

        bboxes = obb2poly(bboxes, version=self.angle_version)
        bboxes = bboxes.reshape(-1, 4, 2)

        gt_rbboxes = obb2poly(gt_rbboxes, version=self.angle_version)
        gt_rbboxes = gt_rbboxes.reshape(-1, 4, 2)

        kld = self.kld_mixture2single(gt2gaussian(bboxes), gt2gaussian(gt_rbboxes))
    ...
    def kld_mixture2single(self, g1, g2):
        ...
        # p_mu = g1.mu  # shape (N, 1, 2)
        # p_var = g1.var  # shape (N, 1, 2, 2)
        ...
        t_mu, t_var = g2  # shape (num_gt, 2), (num_gt, 2, 2)
        K = t_mu.shape[0]
        t_mu = t_mu.unsqueeze(1)  # (num_gt, 1, 2)
        t_var = t_var.unsqueeze(1)  # (num_gt, 1, 2, 2)

        delta = (p_mu - t_mu).unsqueeze(-1)  # (num_gt, num_proposal, 2, 1)
        t_inv = torch.inverse(t_var)  # (num_gt, 1, 2, 2)
