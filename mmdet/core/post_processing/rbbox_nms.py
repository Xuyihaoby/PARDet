...
def multiclass_nms_r(multi_bboxes,
    ...
    if isinstance(nms_cfg['iou_threshold'], float):
        if nms_version == 'v1':
            if 'version' in nms_cfg.keys():
                del nms_cfg['version']
            dets, keep = batched_rnms(bboxes, scores, labels, nms_cfg, class_agnostic=class_agnostic)
        elif nms_version == 'v3':
            dets, keep = obb_batched_nms(bboxes, scores, labels, nms_cfg.iou_threshold, class_agnostic=class_agnostic)
        elif nms_version == 'v2':
            ...
            if return_inds:
                keep = keep[:max_num]
                _, score_indice = scores.sort(descending=True)
                return torch.cat([bboxes, scores[:, None]], 1), labels, keep[score_indice]
            else:
