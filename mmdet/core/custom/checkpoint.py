...
def load_arc(model,
    ...
    conv3x3 = [k for k in state_dict.keys() if "layer1" not in k and 'conv2' in k]
    for table_key in conv3x3:
        table_pretrained =state_dict[table_key]
        table_current = model.state_dict()[table_key]
        times = table_current.shape[0]
        state_dict[table_key] = table_pretrained.unsqueeze(0).repeat(times, 1, 1, 1, 1)

    # load state_dict
    load_state_dict(model, state_dict, strict, logger)
    return checkpoint

def load_arc_convnext(model,
             filename,
             map_location='cpu',
             strict=False,
             logger=None):
    ...
    # interpolate position bias table if needed
    conv3x3 = [k for k in state_dict.keys() if "stages.0" not in k and 'dw' in k and 'weight' in k]
    for table_key in conv3x3:
        table_pretrained =state_dict[table_key]
        table_current = model.state_dict()[table_key]
        times = table_current.shape[0]
...
def load_od_conv(model,
    ...
    # interpolate position bias table if needed
    conv3x3 = [k for k in state_dict.keys()]
    for table_key in conv3x3:

        if 'bn' in table_key or 'downsample' in table_key:
            continue
        n = model.state_dict()[table_key].shape[0]
        state_dict[table_key] = state_dict[table_key].unsqueeze(0).repeat(n, 1, 1, 1,1)
        # import pdb
        # pdb.set_trace()
        # table_current = model.state_dict()[table_key]
        # a, b, c, d, e = table_current.shape
        # state_dict[table_key] = table_current.reshape(a, b, c, d, e)
        # times = table_current.shape[0]
        # state_dict[table_key] = table_pretrained.unsqueeze(0).repeat(times, 1, 1, 1, 1)
    ...
    for table_key in focal_layers_keys:
        ...
        if len(table_pretrained.shape) != 4:
            ...
            if fsize1 < fsize2:
                ...
                table_pretrained_resized[:, :, (fsize2 - fsize1) // 2:-(fsize2 - fsize1) // 2,
                (fsize2 - fsize1) // 2:-(fsize2 - fsize1) // 2] = table_pretrained
                state_dict[table_key] = table_pretrained_resized
            elif fsize1 > fsize2:
                table_pretrained_resized = table_pretrained[:, :, (fsize1 - fsize2) // 2:-(fsize1 - fsize2) // 2,
