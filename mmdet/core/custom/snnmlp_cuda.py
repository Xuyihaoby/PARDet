...
class _lif_h(Function):
    ...
    def forward(ctx, input, tau, vth, lif, dim):
        ...
        flag = torch.zeros_like(input).type(
            torch.bool)  # input.new(batch_size, channels, height, width).type(torch.bool)
        ...
        with torch.cuda.device_of(input):
            ...
            f(block=(CUDA_NUM_THREADS, 1, 1),
              ...
              args=[input.data_ptr(), tau.data_ptr(), vth.data_ptr(), output.data_ptr(), flag.data_ptr(),
                    tmpo.data_ptr()],
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
                  args=[grad_output.data_ptr(), tau.data_ptr(), flag.data_ptr(), tmpo.data_ptr(), grad_input.data_ptr(),
                        grad_tau.data_ptr()],
                  stream=Stream(ptr=torch.cuda.current_stream().cuda_stream))
